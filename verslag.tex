...
\subsection{Messages}
Een belangrijk aspect van IO is het door Smalltalk geinspireerde message-passing. Een expressie geeft haar resultaat door aan de volgende expressie. Dit resultaat kan dan gebruikt worden door deze expressie. Op deze manier kunnen programmas worden opgebouwd uit opeenvolgingen van expressies. Neem bijvoorbeeld dit eenvoudig programma:
\begin{lstlisting}[frame=single]
6 +(3) print
\end{lstlisting}
De eerste expressie is eenvoudigerweg het nummer '6'. Dit nummer word als message doorgegeven aan de volgende expressie \('+(3)'\). Deze expressie telt 3 op bij aan deze doorgegeven message. Het resultaat daarvan, in dit geval het nummer '9', word daarna weer verder gegeven aan de volgende expressie. De laatste expressie, 'print', ontvangt deze message en zet deze op de standaard IOstream.\par
...
Maar voor Io is het belangrijk dat het resultaat van de expressie doorgegeven moet worden aan de volgende expressie. Daarom introduceren we een message 'm'.
...
\begin{lstlisting}
	m ::= symbol | Nil
\end{lstlisting}
Deze bevat of het resultaat van een expressie, ofwel een 'Nil' als er geen resultaat is, of als een expressie dit expliciet wil doorgeven. We moeten nu de standaard state-transitie aanpassen. We voegen het resultaat toe:
\[\langle m,S,s \rangle \rightarrow \langle \prime m,\prime s \rangle\]
Daarnaast moeten we ook vastleggen dat het programma begint met \[m = Nil\]. Het eenvoudigste voorbeeld is de semantiek van de terminator:
\[[Terminator] = \langle m, terminator, s\rangle \rightarrow \langle Nil, s\rangle\]
